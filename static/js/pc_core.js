...
var initDatePicker = function () {
    ...
    var opts = {
        ...
        separator: ' 至 ',
        ranges: {
            '今天': [moment(), moment().add(0, 'days')],
            '最近三天': [moment().subtract(2, 'days'), moment().add(0, 'days')],
            '最近七天': [moment().subtract(6, 'days'), moment().add(0, 'days')]
        },
        locale: {
            applyLabel: '确认',
            cancelLabel: '清除',
    ...
    function changeTabs(initPages, callback) {
        ...
        var change = function () {
            ...
            if (index && typeof initPages[index] == 'function') {
                ...
                //  console.log(content.eq(index),index);
                    ...
                    if (response.error != 0) {
                        if (response.code == '0-1' || response.code == '0-4') {
                            ...
                            if (String(url).indexOf('zj-bonus') > -1) {
                            	...
                            	if(String(window.location.pathname).indexOf('/userAccount') > -1){
									if (String(response.message).indexOf('取款被冻结') > -1) {
								            App.alert('warning', '提示消息', (response.message != '' ? '用户取款异常，请联系客服查询处理谢谢！' : '失败，原因未知'), 5000);
								       }else{
